...
from .models import Member #def subject 안에 member가 정의되지 않았다고 해서 적음
import requests
...
def subject(request):
    ...
    if data_dic['user_id']:
        ...
        data_dic['articles']=Article.objects.filter(subject1 = data_dic['userinfo'].suvject1)[::-1][:20]
...
def articlereg(request):
    ...
    url = f"https://arxiv.org/list/{field_var}/current?skip=0&show={resultSize_var}"
